...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
        ...
        for (int index_iAS = 0; index_iAS < unordered.size(); index_iAS++) {
            System.out.println(
                "champ at index " + champIndex(unordered, index_iAS) + " with value " + list_iAS.get(index_iAS)
    ...
    /**
      ...
      @return ??
     */
     private int champIndex( ArrayList<Integer> challengers, int index) {
        Integer currentValue = challengers.get(index);
        int indexOfCurrent = index;
        Integer champ = currentValue;
        int indexOfChamp = index;
        for (int index_iC = index + 1; index_iC < challengers.size(); index_iC++) {
            if ( challengers.get(index_iC) < champ) {
                champ = challengers.get(index_iC);
                indexOfChamp = index_iC;
            }
        }
        challengers.set(indexOfCurrent, champ);
        challengers.set(indexOfChamp, currentValue);
        list_iAS.add(champ);
        return indexOfChamp;
     }
